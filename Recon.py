...
if len(sys.argv) != 2:
    print "Usage: Recon.py <targets.txt>"
    print "Usage: Recon.py ./results/targets.txt"
    sys.exit(0)
    ...
    for serv in serv_dict:
        ...
        if ("ftp" in serv) or ("tftp" in serv):
            for port in ports:
                ...
                f.write("    hydra -L /usr/share/seclists/Usernames/Names/names.txt -P /usr/share/seclists/Passwords/Common-Credentials/10k_most_common.txt -f -o %s/%s_ftphydra -u %s -s %s ftp \n" % (outputdir, ip_address, ip_address, port))
            ...
            for port in ports:
                ...
                f.write("    wpscan --url http://%s -e ap,vt,cb,u | tee %s/%s_wpscan_%s \n" % (ip_address, outputdir, ip_address, port))
                f.write("    wpscan --url http://%s -P /usr/share/seclists/Passwords/Common-Credentials/10k-most-common.txt -U admin | tee %s/%s_wpscan_brute_%s \n" % (ip_address, outputdir, ip_address, port))
                f.write("    gobuster dir -s '200,204,301,302,307,401,403' -e -l -x php,txt,apsx,asp,html -u http://%s:%s/ -w /usr/share/wordlists/dirbuster/directory-list-lowercase-2.3-medium.txt -t 10 | tee %s/%s_gobuster_ext_%s \n" % (ip_address, port, outputdir, ip_address, port))
                f.write("    gobuster dir -s '200,204,301,302,307,401,403' -e -l -u http://%s:%s/ -w /usr/share/seclists/Discovery/Web-Content/common.txt | tee %s/%s_gobuster_common_%s \n" % (ip_address, port, outputdir, ip_address, port))
                f.write("    gobuster dir -s '200,204,301,302,307,401,403' -e -l -u http://%s:%s/ -w /root/offsecfw/wordlists/dicc.txt | tee %s/%s_gobuster_dicc_%s \n" % (ip_address, port, outputdir, ip_address, port))
                f.write("    gobuster dir -s '200,401' -e -l -u http://%s:%s/ -w /root/offsecfw/wordlists/all.txt | tee %s/%s_gobuster_all_%s \n" % (ip_address, port, outputdir, ip_address, port))
            ...
            for port in ports:
                ...
                f.write("    nmap -n -sV -Pn -p%s --script=http-shellshock-spider -oN '%s/%s_http_shellshock_%s.nmap' %s \n" % (port, outputdir, ip_address, port, ip_address))
                f.write("    nikto -h %s -p %s | tee %s/%s_nikto_https_%s.txt \n" % (ip_address, port, outputdir, ip_address, port))
                f.write("    whatweb --color=never --no-errors https://%s:%s | tee %s/%s_whatweb_https_%s \n" % (ip_address, port, outputdir, ip_address, port))
                f.write("    wpscan --url https://%s -e ap,vt,cb,u | tee %s/%s_wpscan_https_%s \n" % (ip_address, outputdir, ip_address, port))
                f.write("    wpscan --url https://%s -P /usr/share/seclists/Passwords/Common-Credentials/10k_most_common.txt -U admin | tee %s/%s_wpscan_https_brute_%s \n" % (ip_address, outputdir, ip_address, port))
                f.write("    gobuster dir -s '200,204,301,302,307,401,403' -k -e -l -x php,txt,apsx,asp,html -u https://%s:%s/ -w /usr/share/wordlists/dirbuster/directory-list-lowercase-2.3-medium.txt -t 10 | tee %s/%s_gobuster_https_ext_%s \n" % (ip_address, port, outputdir, ip_address, port))
                f.write("    gobuster dir -s '200,204,301,302,307,401,403' -k -e -l -u https://%s:%s/ -w /usr/share/seclists/Discovery/Web-Content/common.txt | tee %s/%s_gobuster_https_common_%s \n" % (ip_address, port, outputdir, ip_address, port))
                f.write("    gobuster dir -s '200,204,301,302,307,401,403' -k -e -l -u https://%s:%s/ -w /root/offsecfw/wordlists/dicc.txt | tee %s/%s_gobuster_https_dicc_%s \n" % (ip_address, port, outputdir, ip_address, port))
                f.write("    gobuster dir -s '200,401' -k -e -l -u https://%s:%s/ -w /root/offsecfw/wordlists/all.txt | tee %s/%s_gobuster_https_all_%s \n" % (ip_address, port, outputdir, ip_address, port))
                f.write("    medusa -U /usr/share/wordlists/metasploit/http_default_users.txt -P /usr/share/wordlists/metasploit/http_default_pass.txt -e ns -h %s - %s -M http -m DIR:secret -f\n" % (ip_address, port))
        elif "apanil" in serv:
            for port in ports:
                # port = port.split("/")[0]
            ...
            for port in ports:
                ...
                f.write("    smbmap -u guest -H %s -R | tee %s/%s_smbmap \n" % (ip_address, outputdir, ip_address))
        elif "ldap" in serv:
            for port in ports:
                # port = port.split("/")[0]
                f.write("[*] Found LDAP service on %s:%s\n" % (ip_address, port))
                f.write("   [>] Use nmap scripts or ldapsearch for further LDAP enumeration, e.g\n")
                f.write("    nmap -n -sV -Pn -p%s --script=ldap-search.nse -oN '%s/%s_ldap_%s.nmap' %s \n" % (port, outputdir, ip_address, port, ip_address))
                f.write("    ldapsearch -H ldap://%s -x -LLL -s base -b "" supportedSASLMechanisms | tee %s/%s_ldapsearch_%s \n" % (ip_address, outputdir, ip_address, port))
                f.write("    ldapdomaindump -u 'DOMAIN\\user' %s \n" % (ip_address))
                f.write("    bloodhound.py -u user -p pass -d DOMAIN -ns %s -dc hostname.domain \n" % (ip_address))
                f.write("    GetUserSPNs.py -request -dc-ip %s DOMAIN/user:pass | tee %s/%s_ldapspns_%s \n" % (ip_address, outputdir, ip_address, port))
            ...
            for port in ports:
                ...
                f.write("    medusa -U /usr/share/seclists/Usernames/top-usernames-shortlist.txt -P /usr/share/seclists/Passwords/Common-Credentials/best110.txt -e ns -h %s - %s -M ssh -f\n" % (ip_address, port))
            ...
            for port in ports:
                ...
                f.write("    dnsrecon -d example.com -n %s -a | tee %s/%s_dnsrecon_%s \n" % (ip_address, outputdir, ip_address, port))
    f.close()
    print "[*]  Recon completed for %s. Check %s" % (ip_address, outputfile)
